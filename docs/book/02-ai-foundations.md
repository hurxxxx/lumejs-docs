...
이 장은 비개발자가 이해할 수 있는 언어로 **AI 시스템의 핵심 구조**를 설명한다.
...
### 2-2. 데이터의 종류
비개발자 관점에서 중요한 데이터 유형은 다음과 같다.

- **구조화 데이터**: 표, 숫자, 데이터베이스 값
- **비구조화 데이터**: 문서, 이메일, 회의록, 이미지
- **로그/행동 데이터**: 사용 기록, 클릭 기록, 업무 기록

업무에 AI를 적용할 때, 가장 많은 가치를 만드는 것은 **비구조화 데이터**다. 이는 비개발자의 업무가 대부분 문서와 커뮤니케이션에 있기 때문이다.
...
즉, 비개발자 입장에서는 “여러 모델을 고르는 일”보다, **하나의 범용 모델을 어떻게 쓰느냐**가 중요해졌다.
...
비개발자가 기억해야 할 점은, “AI가 똑똑해지는 과정은 결국 **사람의 피드백**에 달려 있다”는 것이다.
...
실무에서는 사람 평가가 가장 중요하다. 특히 비개발자 업무는 **정답이 하나가 아닌 경우**가 많기 때문이다.
...
비개발자가 가장 자주 보는 사례로 **회의록 자동화**를 살펴보자.  
...
이 여섯 가지는 비개발자가 반드시 이해해야 하는 핵심이다.  
