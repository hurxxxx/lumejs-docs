...
이 장은 이후 장들의 전체 논리를 연결하는 “기초 공사”다. 그리고 이 책이 강조하는 **“실험을 넘어 에이전트 스케일로, 비즈니스 프로세스 재설계”**라는 메시지의 이론적 기반을 제공한다.
...
멀티모달은 실무자 업무에 특히 중요하다. 예를 들어, 회의 녹음 파일을 텍스트로 바꾸고 요약하는 것, 제품 사진을 분석해 설명을 작성하는 것, 문서 안의 표를 요약하는 것 등이 가능해진다.
...
즉, AI는 더 이상 “대화형 지식 도구”가 아니라 **업무의 일부를 대신 실행할 수 있는 흐름**을 제공한다. 이 변화는 실무자 업무에도 큰 영향을 준다.
...
특히 실무자 업무(보고서, 회의, 의사결정 지원)는 **인건비 비중이 높기 때문에**, AI의 효율화 효과가 빠르게 보인다.
