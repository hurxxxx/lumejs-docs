...
특히 비개발자에게 AI는 “누르면 답이 나오는 도구”로 보이기 쉽다. 하지만 실제로는 **확률적 결과를 만드는 시스템**이다.  
이 차이를 모르면 잘못된 정보를 그대로 믿거나, 민감한 정보를 무심코 입력하게 된다.

최근 국제 규범은 “AI 리터러시(기본 이해 능력)”를 강조한다.  
EU AI Act는 조직이 직원과 사용자에게 **충분한 AI 리터러시를 확보할 책임**이 있다고 명시한다.  
...
EU AI Act는 AI를 제공하거나 사용하는 조직에게 **직원과 사용자의 리터러시 수준을 높이기 위한 조치**를 요구한다.  
...
AI 사고는 반드시 발생한다. 중요한 것은 **대응 체계**다.
...
비개발자 관점에서 중요한 것은 **평가를 통해 위험을 미리 확인하는 것**이다.
...
특히 비개발자는 프롬프트를 ‘지시문’이 아니라 **업무 기준을 담는 문서**로 생각해야 한다.  
